...
class AuditLog < SyncRecord
  ...
  PROMOTED_COLUMNS = %w[
    event_type schema_version frame_type frame_subtype source_mac transmitter_mac
    receiver_mac bssid destination_bssid ssid signal_dbm noise_dbm frequency_mhz
    channel_flags data_rate_kbps antenna_id tsft channel_number fragment_number signal_status adjacent_mac_hint
    ...
    security_flags risk_score identity_source tags wps_device_name wps_manufacturer
    wps_model_name device_fingerprint handshake_captured sensor_id location_id username
  ].freeze
  ...
  INTEGER_PROMOTED_COLUMNS = %w[
    raw_len frame_control_flags security_flags noise_dbm frequency_mhz channel_flags
    data_rate_kbps antenna_id tsft
  ].freeze
  ...
  PAYLOAD_ONLY_FIELDS = %w[
    raw_frame qos_tid_label fragment_offset ip_id ip_flags tcp_flags tcp_seq tcp_ack
